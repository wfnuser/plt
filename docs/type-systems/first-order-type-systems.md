...
一阶类型化的 λ-演算 被称为 $F1$ 系统。与无类型 λ-演算 相比的主要变化是为 λ-抽象 添加了类型注释(Type Annotation)，使用语法 $λx:A.M$ ，其中 $x$ 是函数参数，$A$ 是其类型，$M$ 是函数的主体。（在类型化的编程语言中，我们可能会包括结果的类型，但在这里不是必需的。）从 $λx.M$ 到 $λx:A.M$ 的步骤是从无类型语言到类型化语言演化的主要进展：绑定变量(Bound Variable)获得类型注释。
